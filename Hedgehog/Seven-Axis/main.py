...
base_target_position = 1.5
main_arm_target_position = 1.5
second_arm_target_position = 2.9
second_arm_target_rotation = 1.1
wrist_target_position = 3.14
wrist_target_rotation = 3.14
gripper_target_position = 1.0
...
second_arm_rotation = second_arm_target_rotation
...
with connect('tcp://localhost:10789', emergency=15) as hedgehog:
    with connect('tcp://hedgehog13.local:10789', emergency=15) as hedgehog2:
        ...
        def shutdown(signal, frame):
            ...
            hedgehog2.set_servo(2, False, 1)
            hedgehog2.set_servo(3, False, 1)
        ...
        def on_one_arg_message(payload):
            ...
            global second_arm_rotation_speed_manual
            ...
            if payload == "b'lift-main-arm'":
                main_arm_speed_manual = 0.02
                second_arm_speed_manual -= 0.02
            ...
            if payload == "b'rotate-left-second-arm'":
                second_arm_rotation_speed_manual = -0.02
            if payload == "b'stop-rotate-left-second-arm'":
                second_arm_rotation_speed_manual = 0.0
            if payload == "b'rotate-right-second-arm'":
                second_arm_rotation_speed_manual = 0.02
            if payload == "b'stop-rotate-right-second-arm'":
                second_arm_rotation_speed_manual = 0.0

            if payload == "b'lower-wrist'":
                wrist_speed_manual = 0.02
            ...
            if payload == "b'rotate-left-wrist'":
                wrist_rotation_speed_manual = -0.02
        ...
        def on_two_arg_message(payload):
            global message_rate
            global second_arm_target_rotation
        ...
        def on_three_arg_message(payload):
            ...
            global second_arm_target_rotation
            global second_arm_rotation_target_speed
            ...
            if component == "b'second-arm-goto":
                ...
                second_arm_target_speed = speed
            if component == "b'second-arm-roto":
                second_arm_target_rotation = position
                second_arm_rotation_target_speed = speed
        ...
        while True:
            ...
            second_arm_rotation_speed = hu.calc_speed(second_arm_rotation, second_arm_target_rotation, second_arm_rotation_speed, second_arm_rotation_target_speed, acceleration*3, deceleration*3)
            ...
            if base_speed + base_speed_manual != 0.0:
                ...
                base_position = max(base_position, 0.1)
                base_position = min(base_position, 2.9)
                hedgehog2.set_servo(0, True, hu.from_rad(base_position))
            ...
            if main_arm_speed + main_arm_speed_manual != 0.0:
                ...
                main_arm_position = max(main_arm_position, 0.87)
                main_arm_position = min(main_arm_position, 2.9)
                hedgehog2.set_servo(1, True, hu.from_rad(main_arm_position))
            if  second_arm_speed + second_arm_speed_manual != 0.0:
                if second_arm_speed_manual != 0.0:
                    second_arm_position = max(0.8, min(3.14, second_arm_position + second_arm_speed_manual))
                    second_arm_target_position = second_arm_position
                else:
                    second_arm_position = max(0.8, min(3.14, second_arm_position + second_arm_speed))
                hedgehog2.set_servo(2, True, hu.from_rad(second_arm_position))

            if second_arm_rotation_speed + second_arm_rotation_speed_manual != 0.0:
                if second_arm_rotation_speed_manual != 0.0:
                    second_arm_rotation = max(0.08, min(3.14, second_arm_rotation + second_arm_rotation_speed_manual))
                    second_arm_target_rotation = second_arm_rotation
                else:
                    second_arm_rotation = max(0.08, min(3.14, second_arm_rotation + second_arm_rotation_speed))
                hedgehog.set_servo(0, True, hu.from_rad(second_arm_rotation))

            if  wrist_rotation_speed + wrist_rotation_speed_manual != 0.0:
                if wrist_rotation_speed_manual != 0.0:
                    wrist_rotation = max(0.08, min(3.14, wrist_rotation + wrist_rotation_speed_manual))
                    wrist_target_rotation = wrist_rotation
                else:
                    wrist_rotation = max(0.08, min(3.14, wrist_rotation + wrist_rotation_speed))
                hedgehog.set_servo(1, True, hu.from_rad(wrist_rotation))

            if  wrist_speed + wrist_speed_manual != 0.0:
                if wrist_speed_manual != 0.0:
                    wrist_position = max(0.08, min(3.14, wrist_position + wrist_speed_manual))
                else:
                    wrist_position = max(0.08, min(3.14, wrist_position + wrist_speed))
                hedgehog.set_servo(2, True, hu.from_rad(wrist_position))
            if bite_gripper:
                gripper_target_position = max(0, min(3.14, gripper_position + delta))
            if release_gripper:
                gripper_target_position = max(0, min(3.14, gripper_position - delta))
            if gripper_target_position != gripper_position:
                hedgehog.set_servo(3, True, hu.from_rad(gripper_target_position))
                gripper_position = gripper_target_position
